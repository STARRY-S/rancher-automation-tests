...
def create_deployment_wl(p_client, ns, img, annotations={}, scale=1, node=None, ports=None):
    ...
    scheduling = {"node": {}}
    workloadAnnotations = {"field.cattle.io/defaultPort": True}
    if node != None:
...
def get_cluster_by_id(client, id):
    ...
    for cluster in res["data"]:
        if cluster["id"] == id:
            return cluster
    assert False, "failed to find the cluster {}".format(id)


def restart_ucc():
    mcm_restart_url = CATTLE_TEST_URL + "/mcm/restart/local"
    head = {'Authorization': 'Bearer ' + ADMIN_TOKEN}
    requests.get(mcm_restart_url, verify=False, headers=head)
    time.sleep(10)
