...
eth0_network = '[{\"name\":\"rancher-flat-network\",\"interface\":\"eth0\"}]'
eth0_anno_key = "v1.multus-cni.io/default-network"
eth1_network = '[{\"name\":\"rancher-flat-network\",\"interface\":\"eth1\"}]'
eth1_anno_key = "k8s.v1.cni.cncf.io/networks"

DEFAULT_MASTER = os.environ.get('RANCHER_TEST_SUBNET_MASTER', "eth0")
...
FLATNETWORK_SERVICE_SUFFIX="-flat-network"
REQUEST_TIMEOUT=5
...
def get_flatnetwork_pod_ip(pod, nic="eth1", ipv6=False):
...
def get_flatnetwork_pod_mac(pod):
...
def get_flatnetwork_pod_events(client, pod, reason="FlatNetworkIPError", limit=1):
...
def create_flatnetwork_subnet(client, cidr, flatMode="macvlan", mode = "bridge",
                         ranges=[], gateway="", routes=[], vlan=0,
                         route_settings={}, ipv6to4=False):
    subnet = read_json_from_resource_dir("flatnetwork", "subnet.json")
    subnet["metadata"]["name"] = random_test_name('test-flatnetwork')
    if ipv6to4:
        subnet["metadata"]["annotations"]["flatnetwork.pandaria.io/ipv6to4"] = "true"
    subnet["spec"]["vlan"] = vlan
    subnet["spec"]["cidr"] = cidr
    subnet["spec"]["flatMode"] = flatMode
    subnet["spec"]["gateway"] = gateway
    subnet["spec"]["master"] = DEFAULT_MASTER
    subnet["spec"]["mode"] = mode
    subnet["spec"]["routes"] = routes
    subnet["spec"]["routeSettings"] = route_settings
    subnet["spec"]["ranges"] = ranges

    return client.create_flatnetwork_pandaria_io_flatnetworksubnet(subnet)


def create_flatnetwork_service(client, ns, wl_label_v, ports, skip_wait=False):
    template = read_json_from_resource_dir("flatnetwork", "service.json")

    template["spec"]["ports"] = ports
    template["spec"]["type"] = "ClusterIP"
    template["metadata"]["name"] = random_test_name('test-svc') + FLATNETWORK_SERVICE_SUFFIX
    template["metadata"]["namespace"] = ns["metadata"]["name"]
    template["metadata"]["annotations"]["k8s.v1.cni.cncf.io/networks"] ="rancher-flat-network"
...
def create_flatnetwork_pod(client, ns, group, image, ip, mac, subnet, mode="dual", skip_wait=False):
    template = read_json_from_resource_dir("flatnetwork", "pod.json")
    pod_name = random_test_name('test-flatnetwork-pod')
    ...
    template["metadata"]["annotations"]["flatnetwork.pandaria.io/ip"] = ip
    template["metadata"]["annotations"]["flatnetwork.pandaria.io/mac"] = mac
    template["metadata"]["annotations"]["flatnetwork.pandaria.io/subnet"] = subnet
...
def ip_in_subnet(ip, cidr):
    if ':' in ip:
        return ip in list(map(str, ipaddress.IPv6Network(cidr).hosts()))
