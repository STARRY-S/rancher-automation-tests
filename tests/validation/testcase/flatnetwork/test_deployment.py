...
def test_upgrade():
    ...
    old_workload = create_flatnetwork_deployment(subnet["metadata"]["name"],
                                                 expected_ip,
                                                 "auto",
                                                 1,
...
def create_flatnetwork_deployment(subnet, ip, mac, replicas, strategy=None, mode="dual",
                              skip_wait=False):
    client = factory["client"]
    ns_name = factory['ns']["metadata"]["name"]
    name = random_test_name("busybox")
    template = read_json_from_resource_dir("flatnetwork", "deployment.json")
    # set metadata
