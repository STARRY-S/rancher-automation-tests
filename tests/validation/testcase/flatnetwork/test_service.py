...
def test_deployment_auto():
    ...
    pod1_ip = get_flatnetwork_pod_ip(busybox_pods[0])
    pod2_ip = get_flatnetwork_pod_ip(busybox_pods[1])
    pod_ips = (pod1_ip, pod2_ip)

    # validate flatnetwork service by endpointslice
    flatnetwork_svc_name = simple_svc["metadata"]["name"] + FLATNETWORK_SERVICE_SUFFIX
    flatnetwork_svc_id = simple_svc["metadata"]["namespace"] + "/" + flatnetwork_svc_name
    flatnetwork_svc = client.by_id_service(flatnetwork_svc_id)
    assert flatnetwork_svc is not None

    time.sleep(1)

    label_selector = f"kubernetes.io/service-name={flatnetwork_svc_name}"
    ...
    # validate flatnetwork service by fqdn
    assert sorted(get_ips_from_svc(busybox_pods[0], flatnetwork_svc)) == sorted(pod_ips)
...
def test_statefulset_auto():
    ...
    # create deployment and service
    busybox_workload = create_flatnetwork_statefulset(subnet["metadata"]["name"],
                                                 "auto", "auto", 2)
    wait_for(lambda: "readyReplicas" in client.reload(busybox_workload).status and \
             client.reload(busybox_workload).status.readyReplicas == 2,
    ...
    # validate flatnetwork service by endpointslice
    flatnetwork_svc_name = busybox_workload["metadata"]["name"] + FLATNETWORK_SERVICE_SUFFIX
    flatnetwork_svc_id = busybox_workload["metadata"]["namespace"] + "/" + flatnetwork_svc_name
    flatnetwork_svc = client.by_id_service(flatnetwork_svc_id)
    assert flatnetwork_svc is not None
...
def test_daemonset_auto():
    ...
    for pod in busybox_pods:
        pod_ips.append(get_flatnetwork_pod_ip(pod))
...
def test_auto_create_by_pod():
    ...
    pod = create_flatnetwork_pod(client, ns, group, BUSYBOX_IMAGE, expected_ip, "auto",
                             subnet["metadata"]["name"])

    flatnetwork_svc_name = simple_svc["metadata"]["name"] + FLATNETWORK_SERVICE_SUFFIX
    flatnetwork_svc_id = ns["metadata"]["name"] + "/" + flatnetwork_svc_name
    flatnetwork_svc = client.by_id_service(flatnetwork_svc_id)
    assert flatnetwork_svc is not None
...
def test_custom():
    ...
    expected_ip = get_random_ips_from_cidr(subnet["spec"]["cidr"], 1)[0]
    flatnetwork_svc = create_flatnetwork_service(client, ns, group, svc_ports)
    pod = create_flatnetwork_pod(client, ns, group, BUSYBOX_IMAGE, expected_ip, "auto",
                             subnet["metadata"]["name"])
    flatnetwork_svc_name = flatnetwork_svc["metadata"]["name"]
