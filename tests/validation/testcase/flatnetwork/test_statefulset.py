...
def test_auto():
    ...
    ordered_workload = create_flatnetwork_statefulset(subnet["metadata"]["name"],
                                                 "auto",
                                                 "auto",
                                                 2)
    ...
    parallel_workload = create_flatnetwork_statefulset(subnet["metadata"]["name"],
                                                   "auto",
                                                   "auto",
                                                   2,
                                                   parallel=True)
...
def test_ordered_upgrade():
    ...
    old_workload = create_flatnetwork_statefulset(subnet["metadata"]["name"],
                                              expected_ip,
                                              "auto",
                                              1)
    old_pod = get_common_workload_pods(client, old_workload)[0]
    old_ip = get_flatnetwork_pod_ip(old_pod)
    ...
    new_ip = get_flatnetwork_pod_ip(new_pod)
...
def create_flatnetwork_statefulset(subnet, ip, mac, replicas, parallel=False,
    ...
    template = read_json_from_resource_dir("flatnetwork", "statefulset.json")
    # set metadata
...
def setup_module(request):
    ...
    factory["subnet"] = create_flatnetwork_subnet(factory["client"], subnet_cidr, vlan=vlan)
