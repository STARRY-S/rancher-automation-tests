...
def test_upgrade():
    ...
    expected_ips = get_random_ips_from_cidr(subnet["spec"]["cidr"], len(worker_nodes))
    old_workload = create_macvlan_daemonset(subnet["metadata"]["name"],
                                            '-'.join(expected_ips),
                                            "auto")
...
def create_macvlan_daemonset(subnet, ip, mac, mode="dual", skip_wait=False):
    client = factory["client"]
    ns_name = factory["ns"]["metadata"]["name"]
    name = random_test_name("busybox")
    template = read_json_from_resource_dir("macvlan", "macvlan_daemonset.json")
    # set metadata
