...
def test_auto():
    ...
    job = create_macvlan_job(subnet["metadata"]["name"],
                             "auto",
                             "auto")

    busybox_pods = get_macvlan_job_pods(job)
...
def test_fixed_mac():
    ...
    wait_for_job(job)
    pod_mac = get_macvlan_pod_mac(busybox_pods[0])
...
def create_macvlan_job(subnet, ip, mac, mode="dual", skip_wait=False):
    client = factory["client"]
    ns_name = factory['ns']["metadata"]["name"]
    name = random_test_name("busybox")
    template = read_json_from_resource_dir("macvlan", "macvlan_job.json")
    # set metadata
